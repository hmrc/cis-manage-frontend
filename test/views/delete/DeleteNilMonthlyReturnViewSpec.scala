...
class DeleteNilMonthlyReturnViewSpec extends SpecBase {
  ...
  "DeleteNilMonthlyReturnView" - {
    ...
    "must render the page with the correct html elements (English) including monthYear" in new Setup {
      ...
      doc.title must include(messagesEn("delete.deleteNilMonthlyReturn.title", monthYearEn))
      doc.title must include(monthYearEn)

      doc.select("h1").text must include(messagesEn("delete.deleteNilMonthlyReturn.heading", monthYearEn))
    ...
    "must render the page with the correct html elements (Welsh) including monthYear" in new Setup {
      ...
      doc.title must include(messagesCy("delete.deleteNilMonthlyReturn.title", monthYearCy))
      doc.title must include(monthYearCy)

      doc.select("h1").text must include(messagesCy("delete.deleteNilMonthlyReturn.heading", monthYearCy))
